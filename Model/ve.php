...
{
    $sql = "SELECT ve.id, ve.gia_ve, ve.ngay_dat, ve.ghe, ve.id_user, ve.id_kgc, ve.id_bill, ve.tt_ve, users.ten_user, khung_gio_chieu.gio_chieu,khung_gio_chieu.phong_phim, xuat_chieu.ngay_chieu, phim.ten_phim FROM ve INNER JOIN users ON users.id = ve.id_user INNER JOIN khung_gio_chieu ON khung_gio_chieu.id = ve.id_kgc INNER JOIN xuat_chieu ON khung_gio_chieu.id_xuat_chieu = xuat_chieu.id INNER JOIN phim ON phim.id = xuat_chieu.id_phim";
