...
{
    $sql = "SELECT ve.id,bill.thanh_tien, ve.ngay_dat, ve.ghe, ve.id_user, ve.id_kgc, ve.id_bill, ve.tt_ve, users.ten_user,xuat_chieu.ngay_chieu, khung_gio_chieu.gio_chieu FROM ve INNER JOIN users ON users.id = ve.id_user INNER JOIN khung_gio_chieu ON khung_gio_chieu.id = ve.id_kgc INNER JOIN xuat_chieu ON xuat_chieu.id = khung_gio_chieu.id_xuat_chieu INNER JOIN bill ON ve.id_bill = bill.id WHERE users.id =". $id;
    $listve = pdo_query($sql);
    return $listve;
}